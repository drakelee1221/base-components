	...
	private Pair<String, String> locationHeader(RequestContext ctx) {
		...
		if (ctx.getZuulResponseHeaders() != null) {
			for (Pair<String, String> pair : ctx.getZuulResponseHeaders()) {
				if (RpcHeaders.EXTERNAL_REDIRECT_HEADER.equalsIgnoreCase(pair.first())
					&& Boolean.TRUE.toString().equalsIgnoreCase(pair.second())) {
					return null;
				}
				if (LOCATION_HEADER.equalsIgnoreCase(pair.first())) {
					p = pair;
				}
			}
		}
